...
public final static int ABSOLUTE_MIN_VALUE = Integer.MIN_VALUE+6; // coordinate values >= ABSOLUTE_MIN_VALUE will be treated as absolute values
			...
			if (suffix == -1 ||( suffix != name.length()-4 && !name.toLowerCase().endsWith(".jpeg") )) {
				...
				for (int i = 0; i < type.length; i++) {
					if ((o=IMAGES.get(name+type[i])) != null) {
						System.out.println("Putting image in " + name);
	...
	private static void reloadArtwork (final String currentDir, String path) throws IOException  {
		String folderPrefix = "rulesets"+File.separator+"Decker"+File.separator+"artwork";
		...
		final String[] files = mgr.list((currentDir.length()==0) ? folderPrefix : folderPrefix +File.separator+ currentDir);
		
		System.out.println("  Found " + files.length + " files in " + folderPrefix +File.separator+ currentDir);
		
		final String path_prefix = (currentDir.length()==0) ? "/" : (currentDir + "/");
		for (int i = 0; i < files.length; i++) {
			String subFilePath = (currentDir.length()==0) ? files[i] : currentDir + File.separator + files[i];

			String[] subFiles = mgr.list(folderPrefix + File.separator + subFilePath);
			System.out.println("    Found " + subFiles.length + " files in " + folderPrefix + File.separator + subFilePath);
			if (subFiles.length > 0)
			{
				System.out.println("Reading sub files from " + subFilePath);
				...
				if (pos > -1) {
					...
					final String file_name = files[i].substring(0, pos).toLowerCase();
